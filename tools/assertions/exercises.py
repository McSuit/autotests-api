...
def assert_exercise(actual: ExerciseSchema, expected: ExerciseSchema) -> None:
    ...
    Сверяет все поля двух объектов ExerciseSchema:
    ``id``, ``title``, ``course_id``, ``max_score``, ``min_score``,
    ``description``, ``estimated_time`` и ``order_index``.
...
def assert_update_exercise_response(
    ...
    Сверяет все поля, переданные в запросе на обновление:
...
def assert_exercise_not_found_response(actual: InternalErrorResponseSchema) -> None:
    """
    Проверяет, что ответ содержит ошибку отсутствия задания.

    Сравнивает фактический ответ API с ожидаемым сообщением об ошибке
    ``"Exercise not found"``.

    :param actual: Фактический ответ API с ошибкой.
    :raises AssertionError: Если фактический ответ не соответствует ошибке «Задание не найдено».
    """
...
def assert_get_exercises_response(
    ...
    Сверяет длину списка заданий, затем поочерёдно проверяет каждое задание
    с помощью функции ``assert_exercise``.
