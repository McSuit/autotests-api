...
def assert_create_course_response(
    ...
    Помимо скалярных полей проверяет, что вложенная модель файла содержит
    идентификатор, переданный в ``preview_file_id``, а вложенная модель
    пользователя — идентификатор из ``created_by_user_id``.
